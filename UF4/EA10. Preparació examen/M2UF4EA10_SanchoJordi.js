// Exerici 1. Insercions
db.createCollection("books");

// Insert de books_array.json amb un insertMany
db.books.insertMany([
  {
    "_id": 1,
...
// Inserts del document dadesInserts
// Exercici1 a
db.books.insertOne({
...
// a) Cerca tots els llibres que el seu titol no comencin per vocal. Ensenya només el títol.
db.books.find({title: {$not: /^([aeiou])/i}}, {title: 1, _id: 0})

// b) Mostra els llibres que el camp "longDescription" acabi amb la lletra "e" però mostra només el camp ISBN.
db.books.find({longDescription: /e$/}, {isbn: 1, _id: 0})

// c) Mostra tots els llibres que tinguin entre 200 i 500 pàgines (ambdues incloses). Mostra el resultat ordenat pel camp _id de manera descendent i en format bonic.
db.books.find({pageCount: {$gte: 200, $lte: 500}}).sort({_id: -1}).pretty()

/* d) Mostra els documents que tinguin dins el camp "categories" la categoria Mobile o Java. Limita
que es veguin només 15 documents.*/
db.books.find({categories: {$in: ["Mobile", "Java"]}}).limit(15)

// e) Mostra els llibres que no tinguin com autors ni "Anthony Briggs" ni "Adele Goldberg". Mostra només el camp "authors".
db.books.find({authors: {$nin: ["Anthony Briggs", "Adele Goldberg"]}}, {authors: 1, _id: 0})

// f) Cerca tots els llibres que continguin les paraules Action o Applications al seu títol. Has de tenir en compte el case sensitive.
db.books.find({title: /Action|Applications/i})

// g) Mostra tots els llibres que tinguin 8 authors. Mostra'ls en un format "bonic"
db.books.find({authors: {$size: 8}}).pretty();


// Exercici 3. Actualitzacions (2 punts)
// a) Modifica l'autor "Joe Lennon" del llibre amb titol "HTML5 in Action", el nou nom és "Rob Pen".
db.books.updateOne({title: "HTML5 in Action"}, {$set: {"authors.1": "Rob Pen"}})

// b) Modifica el tercer autor del llibre amb titol "Portlets and Apache Portals"", el tercer autor s'ha de dir ""Tim Robinson".
db.books.updateOne({title: "Portlets and Apache Portals"}, {$set: {"authors.2": "Tim Robinson"}})

// c) Afegeix un altre autor anomenat "Sam Watters", al llibre "Code Generation in Action"".
db.books.updateOne({title: "Code Generation in Action"}, {$push: {authors: "Sam Watters"}})

// d) Afegeix un nou camp anomenat "Assessment" a tots els llibres que la seva categoria sigui "Internet" i el status sigui "Publish". 
//El valor d'aquest camp serà de 3000. Has de tenir en compte el case sensitive.
db.books.updateMany({categories: /internet/i, status: /publish/i}, {$set: {"assessment": 3000}})


// Exercici 4. Eliminacions (1,2 punts)
// a) Elimina tots els llibres que en el títol continguin la paraula "Development". Has de tenir en compte el case sensitive.
db.books.deleteMany({title: /development/i})

// b) Elimina l'autor "Chris Gray" del camp authors de llibre amb titol "Ajax in Practice".
db.books.updateOne({title: "Ajax in Practice"}, {$pull: {authors: "Chris Gray"}})

// c) Elimina el camp "thumbnailUrl" de tots els documents de la col-lecció.
db.books.updateMany({}, {$unset: {thumbnailUrl: ""}})


// Exercici 5. Agregacions (3 punts)
/*a) Mostra tots els llibres que tinguin com a minim 5 authors. Utilitza l'estructura aggregate. 
Els llibres s'han de mostrar amb el següent format:
{"_id": "Ajax in Practice", "num_autors":5}*/
db.books.aggregate([
  {$project: {title: 1, num_authors: {$size: "$authors"}}},
  {$match: {num_authors: {$gte: 5}}}
])

// b) Mostra quans llibres ha escrit cada autor. Utilitza l'estructura aggregate.
db.books.aggregate([
  {$unwind: "$authors"},
  {$group: {_id: "$authors", count: {$sum: 1}}}
])

// c) Ordena els llibres per número de categories a que pertany de forma descendent. 
...
db.books.aggregate([
  {$project: {title: 1, num_categories: {$size: "$categories"}}},
  {$sort: {num_categories: -1}},
  {$limit: 1}
])

// d) Mostra els ISBN per cada Status "status".
db.books.aggregate([
  {$group: {_id: "$status", isbn: {$push: "$isbn"}}}
])
